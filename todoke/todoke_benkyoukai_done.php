...
//  データベース接続
	...
	$sql='INSERT INTO dat_todoke(todokecode,timecard,shaincode,name,kubun,date1,jiyu) VALUES (?,?,?,?,?,?,?)';
	$stmt=$dbh->prepare($sql);
    $dbdata[]=6;
    $dbdata[]=$db_id;
	$dbdata[]=$db_scode;
...
//  メール 本文  //    
    ...
    $honbun.='社員名：'.$db_name."\n\n";
    $honbun.='区分：研修会('.$db_kubun.")\n\n";
    $honbun.='研修日：'.date('Y年m月d日',strtotime($db_date1));
    $honbun.='事由：'.$db_jiyu."\n\n";
