...
//  データベース接続
	...
	$sql='INSERT INTO dat_todoke(todokecode,timecard,shaincode,name,kubun,date1,time1,jiyu) VALUES (?,?,?,?,?,?,?,?)';
	$stmt=$dbh->prepare($sql);
    $dbdata[]=3;
    $dbdata[]=$db_id;
	$dbdata[]=$db_scode;
...
//  メール 本文  //    
    ...
    $honbun.='社員名：'.$db_name."\n\n";
    $honbun.='区分：'.$db_kubun."\n\n";
    $honbun.='日時：'.date('Y年m月d日',strtotime($db_date1));
    $honbun.='     '.$db_time1."\n\n";
    $honbun.='事由：'.$db_jiyu."\n\n";
