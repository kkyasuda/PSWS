...
<!--- 届け種類 --->
          ...
          <form name="checksend">
            ...
            <div class="modal-body">
                <label class="col-sm-2 control-label"></label>
                <div id="print-err1"></div>
                <label class="col-sm-2 control-label"></label>
                <div id="print-err2"></div>
                <label class="col-sm-2 control-label"></label>
                <div id="print-err3"></div>
                <label class="col-sm-2 control-label"></label>
                <div id="print-err4"></div>
                <label class="col-sm-2 control-label"> </label>
...
$(function(){
    ...
    $('#mcheck').on('show.bs.modal', function (event) {
        ...
        if (input_radio0=='あり' && input_shosai1.length>160){
                        $('#checkshosai1').val("!　文字数が多すぎます。１６０文字まで");
                        err_flag=true;
        }
        if (input_shosai2.length>250){
                        $('#checkshosai2').val("!　文字数が多すぎます。２５０文字まで");
                        err_flag=true;
        }
        if (input_date1==''){
                        $('#checkdate1').val("!　未入力");
    ...
    var fd = new FormData($('#form_input').get(0));
        ...
        $.ajax({
            ...
            dataType: 'json',
                ...
                $('#quitmes').modal();
                $('#print-err1').text(data.err1);
                $('#print-err2').text(data.err2);
                $('#print-err3').text(data.err3);
                $('#print-err4').text(data.err4);
